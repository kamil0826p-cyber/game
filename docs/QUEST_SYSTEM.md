...
Quest rewards support experience, gold, and silver. Item consumption, terminal `REWARDED` status, character progression, currency changes, and ledger entries are committed in one transaction. The unique character/quest relation and terminal status prevent duplicate rewards.

Quest NPC dialogues define roots for `notStarted`, `active`, `ready`, and `rewarded`, plus `ACCEPT` and `TURN_IN` actions. The first seeded quest, `rabbit-fur-for-mira`, asks for five `rabbit-fur`, consumes exactly five, and grants 180 experience and 3 gold. Mira is placed on a validated free Greenfields tile. The quest journal reads live data through `quests:get`; the old hard-coded frontend list was removed.
