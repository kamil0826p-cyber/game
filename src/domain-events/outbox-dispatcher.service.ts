...
import { materializeDomainEffects } from './domain-event.effects.js';
import type { DomainAuditEntry, DomainContribution, DomainEventRecord } from './domain-event.types.js';

interface ClaimedOutbox { id: string; eventId: string; attempts: number; }
...
export class OutboxDispatcherService implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  onApplicationBootstrap(): void {
    ...
    this.timer = setInterval(() => void this.dispatchNow(), this.config.values.OUTBOX_POLL_INTERVAL_MS);
    this.timer.unref?.();
  ...
  private async process(entry: ClaimedOutbox): Promise<void> {
    try {
      await this.events.consumeExactlyOnce('core-ledgers-v1', entry.eventId, async (tx, event) => {
        const effects = materializeDomainEffects(event);
        await this.applyContributions(tx, event, effects.contributions);
        await this.applyAudit(tx, event, effects.audit);
      });
      await this.prisma.$executeRaw(Prisma.sql`
        UPDATE "EventOutbox"
        SET "status" = 'PUBLISHED', "publishedAt" = NOW(), "lockedAt" = NULL,
            "lastError" = NULL, "updatedAt" = NOW()
        WHERE "id" = ${entry.id}::uuid AND "status" = 'PROCESSING'
      `);
    } catch (error) {
  ...
  private async applyAudit(
    tx: Prisma.TransactionClient,
    event: DomainEventRecord,
    entries: readonly DomainAuditEntry[],
  ): Promise<void> {
    for (const entry of entries) {
      await tx.$executeRaw(Prisma.sql`
        INSERT INTO "RewardAuditLedger" (
          "id", "eventId", "operationId", "characterId", "resourceType", "resourceKey",
          "amount", "balanceAfter", "reason", "metadata", "createdAt"
        ) VALUES (
          ${randomUUID()}::uuid, ${event.id}::uuid, ${event.operationId},
          ${entry.characterId ?? null}::uuid, ${entry.resourceType}, ${entry.resourceKey ?? null},
          ${entry.amount}, ${entry.balanceAfter ?? null}, ${entry.reason},
          ${JSON.stringify(entry.metadata ?? {})}::jsonb, NOW()
        )
