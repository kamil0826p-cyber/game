...
export class QuestGateway {
  ...
  constructor(private readonly quests: QuestService, private readonly world: WorldStateService, private readonly localization: LocalizationService) {}

  @SubscribeMessage('quests:get')
  async getQuests(@ConnectedSocket() client: GameSocket, @MessageBody() raw: unknown): Promise<SocketAck<QuestLogSnapshot>> {
    try {
      ...
      return { ok: true, data: await this.quests.getLog(session.characterId, client.data.locale ?? 'en', session.mapId) };
    } catch (error) {
      ...
      this.logger.error('Unhandled quest gateway error.', error instanceof Error ? error.stack : undefined);
