...
function questProgressJson(progress: QuestProgressState): Prisma.InputJsonObject {
  return { counters: { ...progress.counters } };
}
...
export class QuestService {
  ...
  async accept(session: PlayerSession, questKey: string): Promise<QuestMutationResult> {
    const state = await this.prisma.$transaction(async (transaction) => {
      ...
      const progress = questProgressJson(emptyQuestProgress());
  ...
  private async recordObjective(characterId: string, predicate: (step: QuestStepDefinition) => boolean): Promise<void> {
    ...
    for (const quest of active) {
      ...
      const progress = incrementObjectiveProgress(steps, parseQuestProgress(quest.progress), predicate);
      await this.prisma.characterQuest.update({ where: { id: quest.id }, data: { progress: questProgressJson(progress) } });
    }
  }
