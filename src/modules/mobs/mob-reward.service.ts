...
export class MobRewardService {
  ...
  async award(session: PlayerSession, mob: RuntimeMob): Promise<MobRewardSettlement> {
    ...
    session.experience = result.updated.experience;
    session.hp = result.updated.hp;
    session.maxHp = result.updated.maxHp;
    session.energy = result.updated.energy;
    session.maxEnergy = result.updated.maxEnergy;
    session.strength = result.updated.strength;
    session.agility = result.updated.agility;
    session.intelligence = result.updated.intelligence;
    session.armor = result.updated.armor;
    ...
    for (const reward of rewards) {
      ...
      for (let slotIndex = 0; slotIndex < INVENTORY_CAPACITY && remaining > 0; slotIndex += 1) {
        ...
        const created = await transaction.inventoryItem.create({
          data: { characterId, itemDefinitionId: definition.id, quantity, slotIndex },
        });
        items.push(created);
