...
const actionSchema = z.discriminatedUnion('action', [
  z.object({ requestId, combatId: z.string().uuid(), action: z.literal('BASIC_ATTACK') }).strict(),
  z.object({
    requestId,
...
export class MobGateway implements OnGatewayDisconnect {
  ...
  getActive(
    ...
    try {
      listSchema.parse(raw);
      const session = this.requireSession(client);
      return { ok: true, data: this.combat.getActive(session.userId, session.characterId) };
    } catch (error) {
      return { ok: false, error: this.toSocketError(error, client) };
    }
  }
  ...
  async act(
    ...
    try {
      ...
      return {
        ok: true,
        data: await this.combat.act(session.userId, session.characterId, payload.combatId, payload),
      };
    } catch (error) {
      return { ok: false, error: this.toSocketError(error, client) };
    }
