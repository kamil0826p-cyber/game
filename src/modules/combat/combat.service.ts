...
export class CombatService implements OnModuleDestroy {
  ...
  async request(userId: string, characterId: string, targetCharacterId: string): Promise<CombatSnapshot> {
    if (characterId === targetCharacterId) this.fail(GAME_ERROR_CODES.COMBAT_SELF, 'errors.combat.self');
    ...
    return this.withMovementQuiesced(
      ...
      () => this.withActorLocks(lockedActorIds, async () => {
        ...
        const initiatorParty = await this.revalidateFrozenParty(activeInitiator, initialInitiators);
        const recipientParty = await this.revalidateFrozenParty(activeRecipient, initialRecipients);
        ...
        if (policy === 'FORBIDDEN') this.fail(GAME_ERROR_CODES.COMBAT_SAFE_ZONE, 'errors.combat.safeZone');
        ...
        await this.assertNoTrades([...initiatorParty.sessions, ...recipientParty.sessions]);
        ...
        try {
          this.occupancy.reserve(runtime.actors.map((actor) => actor.actorId), combatId);
        } catch {
        ...
        this.notifyWaitingParty(runtime, initiatorParty.sessions, recipientParty.sessions);
        return snapshot;
      }),
    );
  }

  async respond(userId: string, characterId: string, combatId: string, accept: boolean): Promise<CombatSnapshot> {
    return this.withCombatLock(combatId, async () => {
      ...
      if (runtime.recipientActorId !== session.characterId) this.fail(GAME_ERROR_CODES.COMBAT_FORBIDDEN, 'errors.combat.forbidden');
      if (runtime.status !== 'REQUESTED') this.fail(GAME_ERROR_CODES.COMBAT_NOT_ACTIVE, 'errors.combat.notActive');
      ...
      return this.withMovementQuiesced(participants, async () => {
        ...
        for (const participant of participants) this.assertAvailable(participant, combatId);
  ...
  async act(userId: string, characterId: string, combatId: string, command: CombatActionCommand): Promise<CombatSnapshot> {
    return this.withCombatLock(combatId, () => this.resolveAction(userId, characterId, combatId, command));
  }
  ...
  private async revalidateFrozenParty(anchor: PlayerSession, party: CombatParty): Promise<CombatParty> {
    ...
    const currentIds = new Set(currentGroup?.members.map((member) => member.characterId) ?? [anchor.characterId]);
  ...
  private async assertFrozenRoster(runtime: CombatRuntime): Promise<void> {
    for (const team of runtime.teams) {
      ...
      for (const actorId of team.actorIds) {
        ...
        if (participant.realmId !== anchor.realmId || participant.mapId !== anchor.mapId) {
          this.fail(GAME_ERROR_CODES.COMBAT_PARTICIPANT_UNAVAILABLE, 'errors.combat.participantUnavailable');
        }
      }
    }
  ...
  private notifyWaitingParty(runtime: CombatRuntime, initiators: PlayerSession[], recipients: PlayerSession[]): void {
    for (const session of [...initiators, ...recipients]) {
      if (session.characterId === runtime.initiatorActorId || session.characterId === runtime.recipientActorId) continue;
  ...
  private syncRuntimeToWorld(runtime: CombatRuntime): PlayerSession[] {
    ...
    for (const actor of runtime.actors) {
      ...
      if (session.combatState !== combatState || session.hp !== hp || session.energy !== actor.energy) {
  ...
  private async broadcastAndPersist(runtime: CombatRuntime, snapshot: CombatSnapshot, checkpoint: boolean): Promise<void> {
    const sessions = this.syncRuntimeToWorld(runtime);
    for (const session of sessions) this.publisher.emit(session.socketId, 'combat:updated', snapshot);
    await Promise.all(runtime.actors.flatMap((actor) => {
      if (!actor.characterId) return [];
      const cooldowns = Object.fromEntries([...actor.skills].map(([key, skill]) => [key, skill.cooldownTurnsRemaining]));
      return [this.skills.persistCooldowns(actor.characterId, cooldowns).catch((error: unknown) => {
        this.logger.error(`Could not persist combat cooldowns for ${actor.characterId}.`, error instanceof Error ? error.stack : undefined);
      })];
    }));
    ...
    await Promise.all(sessions.map(async (session) => {
      try {
        ...
        this.logger.error(`Could not checkpoint combat state for ${session.characterId}.`, error instanceof Error ? error.stack : undefined);
      }
    }));
  }
  ...
  private publishPublicState(session: PlayerSession): void {
    ...
    for (const watcherId of session.watcherCharacterIds) {
      ...
      if (watcher?.activeInWorld) this.publisher.emit(watcher.socketId, 'world:playerMoved', payload);
    }
  }

  private scheduleRequestExpiry(runtime: CombatRuntime): void {
    this.setTimer(runtime.combatId, Math.max(1, (runtime.expiresAt ?? Date.now()) - Date.now()), () => void this.expireRequest(runtime.combatId));
  }
  ...
  private scheduleTurn(runtime: CombatRuntime): void {
    ...
    this.setTimer(runtime.combatId, Math.max(1, runtime.turnEndsAt - Date.now()), () => {
      void this.withCombatLock(runtime.combatId, async () => {
        ...
        await this.resolveAction(undefined, expectedActorId, current.combatId, { action: 'BASIC_ATTACK' });
      });
    });
  }
  ...
  private requireParticipant(runtime: CombatRuntime, characterId: string, userId: string): PlayerSession {
    if (!runtime.actors.some((actor) => actor.characterId === characterId)) this.fail(GAME_ERROR_CODES.COMBAT_FORBIDDEN, 'errors.combat.forbidden');
