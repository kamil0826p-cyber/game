...
import {
  NarrativeService,
  type NarrativeChoiceMutation,
} from './narrative.service.js';

const getSchema = z
  .object({
    requestId: z.string().min(1).max(64),
    questKey: z.string().min(1).max(96),
  })
  .strict();
const choiceSchema = getSchema
  .extend({
...
export class NarrativeGateway {
  ...
  async choose(
    @ConnectedSocket() client: GameSocket,
    @MessageBody() raw: unknown,
  ): Promise<SocketAck<NarrativeChoiceMutation>> {
    return this.handle(client, async () => {
      const request = choiceSchema.parse(raw);
      const session = this.requireSession(client);
      return this.narrative.choose(
        session,
        request.questKey,
        request.operationId,
        request.optionKey,
  ...
  private async handle<T>(
    ...
    try {
      ...
      this.logger.error(
        'Unhandled narrative gateway error.',
