...
function canonicalValue(value: unknown, propertyName?: string): unknown {
  ...
  for (const key of Object.keys(value).sort()) {
    ...
    if ((key === 'uuid' || key === 'databaseId') && typeof entry === 'string') {
      continue;
    }
...
const byKey = <T extends KeyedRecord>(left: T, right: T): number =>
  left.key.localeCompare(right.key);

function normalizedManifest(manifest: GameContentManifest): GameContentManifest {
  return {
    ...manifest,
    maps: [...manifest.maps].sort(byKey),
    portals: [...manifest.portals].sort(byKey),
    npcs: [...manifest.npcs].sort(byKey),
    quests: [...manifest.quests].sort(byKey),
    mobs: [...manifest.mobs].sort(byKey),
    encounters: [...manifest.encounters].sort(byKey),
    skills: [...manifest.skills].sort(byKey),
    items: [...manifest.items].sort(byKey),
    lootTables: [...manifest.lootTables].sort(byKey),
    recipes: [...manifest.recipes].sort(byKey),
    expeditions: [...manifest.expeditions].sort(byKey),
    modifiers: [...manifest.modifiers].sort(byKey),
  };
}
...
function inBounds(map: { width: number; height: number }, x: number, y: number): boolean {
  return (
    Number.isInteger(x) &&
    Number.isInteger(y) &&
    x >= 0 &&
...
function validateDialogue(
  ...
  for (const root of roots) {
    if (!nodeIds.has(root)) {
      issues.push(`NPC ${npcKey} dialogue references missing root node ${root}.`);
    }
  }
  for (const itemKey of dialogue.merchant?.itemKeys ?? []) {
    if (!itemKeys.has(itemKey)) {
      issues.push(`NPC ${npcKey} merchant references missing item ${itemKey}.`);
    }
  }
  ...
  for (const nodeId of nodeIds) {
    for (const target of dialogueEdges(dialogue, nodeId)) {
      if (!nodeIds.has(target)) {
        issues.push(
          `NPC ${npcKey} dialogue node ${nodeId} references missing node ${target}.`,
        );
      }
    }
    for (const choice of dialogue.nodes[nodeId]?.choices ?? []) {
      ...
      if (questKey && !questKeys.has(questKey)) {
        issues.push(
          `NPC ${npcKey} dialogue choice ${choice.id} references missing quest ${questKey}.`,
        );
      }
    }
  }
  ...
  for (const nodeId of nodeIds) {
    if (!reachable.has(nodeId)) {
      issues.push(`NPC ${npcKey} dialogue contains unreachable node ${nodeId}.`);
    }
  }
  ...
  const visit = (nodeId: string, path: string[]): void => {
    if (visiting.has(nodeId)) {
      ...
      const explicitlyAllowed = cycle.some(
        (entry) => dialogue.nodes[entry]?.allowCycle === true,
      );
      if (!explicitlyAllowed) {
        issues.push(`NPC ${npcKey} dialogue contains cycle ${cycle.join(' -> ')}.`);
      }
      return;
    }
...
function validateSkillCycles(
  ...
  const visit = (key: string, path: string[]): void => {
    if (visiting.has(key)) {
      ...
      issues.push(
        `Skill prerequisites contain cycle ${[
          ...path.slice(Math.max(0, cycleStart)),
          key,
        ].join(' -> ')}.`,
      );
      return;
    ...
    visiting.add(key);
    for (const dependency of skillsByKey.get(key)?.prerequisiteKeys ?? []) {
      visit(dependency, [...path, key]);
    }
    visiting.delete(key);
    visited.add(key);
  };
  for (const key of skillsByKey.keys()) visit(key, []);
}
...
export function validateContentManifest(manifest: GameContentManifest): void {
  ...
  if (manifest.schemaVersion !== 1) {
    issues.push(`Unsupported content schema version ${String(manifest.schemaVersion)}.`);
  }
  for (const section of CONTENT_SECTIONS) {
    assertUniqueKeys(section, manifest[section], issues);
  }
  ...
  if (!maps.has(manifest.defaultMapKey)) {
    issues.push(`Default map ${manifest.defaultMapKey} does not exist.`);
  }
  ...
  const occupy = (mapKey: string, x: number, y: number, owner: string): void => {
    ...
    if (previous) {
      issues.push(
        `Coordinate collision on ${mapKey} at ${coordinate}: ${previous} and ${owner}.`,
      );
    } else {
      occupied.set(coordinate, owner);
    }
  };
  for (const map of manifest.maps) {
    occupy(map.key, map.spawnX, map.spawnY, `map ${map.key} spawn`);
  }
  ...
  for (const portal of manifest.portals) {
    ...
    if (!destination) {
      issues.push(
        `Portal ${portal.key} references missing destination map ${portal.destinationMapKey}.`,
      );
  ...
  for (const lootTable of manifest.lootTables) {
    for (const entry of lootTable.entries) {
      if (!items.has(entry.itemKey)) {
        issues.push(
          `Loot table ${lootTable.key} references missing item ${entry.itemKey}.`,
        );
      }
      ...
      if (
        ...
        issues.push(
          `Loot table ${lootTable.key} has invalid quantity range for ${entry.itemKey}.`,
        );
      }
    }
  }
  ...
  for (const mob of manifest.mobs) {
    ...
    if (!lootTables.has(mob.lootTableKey)) {
      issues.push(`Mob ${mob.key} references missing loot table ${mob.lootTableKey}.`);
    }
    if (!Number.isInteger(mob.level) || mob.level <= 0) {
      issues.push(`Mob ${mob.key} has invalid level.`);
    }
    if (!Number.isInteger(mob.respawnMs) || mob.respawnMs < 0) {
      issues.push(`Mob ${mob.key} has invalid respawnMs.`);
    }
  }

  for (const encounter of manifest.encounters) {
    if (!maps.has(encounter.mapKey)) {
      issues.push(
        `Encounter ${encounter.key} references missing map ${encounter.mapKey}.`,
      );
    }
    if (
      !Number.isInteger(encounter.minimumPlayers) ||
      encounter.minimumPlayers < 1 ||
      encounter.minimumPlayers > 10
    ) {
      issues.push(`Encounter ${encounter.key} has invalid minimumPlayers.`);
    }
    if (
      !Number.isInteger(encounter.maximumPlayers) ||
      encounter.maximumPlayers < encounter.minimumPlayers ||
      encounter.maximumPlayers > 10
    ) {
      issues.push(`Encounter ${encounter.key} has invalid maximumPlayers.`);
    }
    let actorCount = 0;
    for (const actor of encounter.actors) {
      if (!mobs.has(actor.mobKey)) {
        issues.push(
          `Encounter ${encounter.key} references missing mob ${actor.mobKey}.`,
        );
      }
      if (!Number.isInteger(actor.count) || actor.count <= 0) {
        issues.push(
          `Encounter ${encounter.key} has invalid actor count for ${actor.mobKey}.`,
        );
      }
      actorCount += actor.count;
    }
    if (actorCount < 1 || actorCount > 10) {
      issues.push(`Encounter ${encounter.key} must contain 1-10 actors.`);
    }
    for (const modifierKey of encounter.modifiers ?? []) {
      if (!modifiers.has(modifierKey)) {
        issues.push(
          `Encounter ${encounter.key} references missing modifier ${modifierKey}.`,
        );
      }
    }
  }
  ...
  for (const skill of manifest.skills) {
    for (const prerequisite of skill.prerequisiteKeys) {
      if (!skillKeys.has(prerequisite)) {
        issues.push(`Skill ${skill.key} references missing prerequisite ${prerequisite}.`);
      }
      if (prerequisite === skill.key) {
        issues.push(`Skill ${skill.key} cannot depend on itself.`);
      }
    }
  }
  ...
  for (const quest of manifest.quests) {
    if (!Number.isInteger(quest.minimumLevel) || quest.minimumLevel < 1) {
      issues.push(`Quest ${quest.key} has invalid minimumLevel.`);
    }
    const stepIds = new Set<string>();
    for (const step of quest.steps) {
      if (stepIds.has(step.id)) {
        issues.push(`Quest ${quest.key} contains duplicate step ${step.id}.`);
      }
      stepIds.add(step.id);
      if (step.itemKey && !items.has(step.itemKey)) {
        issues.push(`Quest ${quest.key} references missing item ${step.itemKey}.`);
      }
      if (step.mobKey && !mobs.has(step.mobKey)) {
        issues.push(`Quest ${quest.key} references missing mob ${step.mobKey}.`);
      }
      if (step.npcKey && !npcs.has(step.npcKey)) {
        issues.push(`Quest ${quest.key} references missing NPC ${step.npcKey}.`);
      }
      if (
        step.quantity !== undefined &&
        (!Number.isInteger(step.quantity) || step.quantity <= 0)
      ) {
        issues.push(`Quest ${quest.key} step ${step.id} has invalid quantity.`);
      }
    }
    if (
      !nonNegativeInteger(quest.rewards.experience) ||
      !nonNegativeInteger(quest.rewards.silver) ||
      !nonNegativeInteger(quest.rewards.gold)
    ) {
      issues.push(`Quest ${quest.key} contains an illegal negative or fractional reward.`);
    }
    for (const reward of quest.rewards.items ?? []) {
      if (!items.has(reward.itemKey)) {
        issues.push(`Quest ${quest.key} rewards missing item ${reward.itemKey}.`);
      }
      if (!Number.isInteger(reward.quantity) || reward.quantity <= 0) {
        issues.push(`Quest ${quest.key} has invalid item reward quantity.`);
      }
    }
  }
  ...
  for (const recipe of manifest.recipes) {
    for (const entry of [...recipe.ingredients, ...recipe.outputs]) {
      if (!items.has(entry.itemKey)) {
        issues.push(`Recipe ${recipe.key} references missing item ${entry.itemKey}.`);
      }
      if (!Number.isInteger(entry.quantity) || entry.quantity <= 0) {
        issues.push(`Recipe ${recipe.key} has invalid quantity for ${entry.itemKey}.`);
      }
    }
    if (recipe.outputs.length === 0) {
      issues.push(`Recipe ${recipe.key} has no outputs.`);
    }
  }
  ...
  for (const expedition of manifest.expeditions) {
    for (const encounterKey of expedition.encounterKeys) {
      if (!encounters.has(encounterKey)) {
        issues.push(
          `Expedition ${expedition.key} references missing encounter ${encounterKey}.`,
        );
      }
    }
    for (const modifierKey of expedition.modifierKeys) {
      if (!modifiers.has(modifierKey)) {
        issues.push(
          `Expedition ${expedition.key} references missing modifier ${modifierKey}.`,
        );
      }
    }
  }
...
export function diffContentManifests(
  ...
  for (const section of CONTENT_SECTIONS) {
    const before = new Map(
      (previous?.[section] ?? []).map((record) => [record.key, stableStringify(record)]),
    );
    const after = new Map(
      next[section].map((record) => [record.key, stableStringify(record)]),
    );
    result[section] = {
