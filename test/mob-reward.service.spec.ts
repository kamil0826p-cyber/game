...
function session(): PlayerSession {
  return {
    socketId: 'socket-1', userId: 'user-1', characterId: 'char-1', realmId: 'realm-1', realmSlug: 'world-1', mapId: 'map-1',
    name: 'Hero', characterClass: 'WARRIOR', gender: 'MALE', locale: 'pl', level: 1, experience: 0, outfitKey: 'warrior', x: 2, y: 2, direction: 'SOUTH',
    combatState: 'IDLE', hp: 100, maxHp: 100, energy: 100, maxEnergy: 100, strength: 10, agility: 10, intelligence: 10, armor: 10,
    stateRevision: 0, persistedRevision: 0, lastSavedAt: new Date(), lastAcceptedMoveAt: 0, lastCommandSequence: 0, dirty: false, connectedAt: Date.now(), activeInWorld: true,
    movementGeneration: 0, visibleCharacterIds: new Set(), watcherCharacterIds: new Set(),
  };
}
function mob() {
  return {
...
describe('MobRewardService', () => {
  const character = {
    id: 'char-1', userId: 'user-1', level: 1, experience: 0, hp: 100, maxHp: 100, energy: 100, maxEnergy: 100,
    strength: 10, agility: 10, intelligence: 10, armor: 10,
  };
  const updated = {
    level: 1, experience: 150, hp: 100, maxHp: 100, energy: 100, maxEnergy: 100,
    strength: 10, agility: 10, intelligence: 10, armor: 10, stateVersion: 1,
  };
  const definition = {
  ...
  beforeEach(() => {
    tx = {
      character: { findUnique: vi.fn().mockResolvedValue(character), update: vi.fn().mockResolvedValue(updated) },
  ...
  it('persists progression, grants loot and synchronizes the online session', async () => {
    ...
    const service = new MobRewardService(prisma as never, domainEvents as never, quests as never);
    const reward = await service.award(player, mob());
    expect(prisma.$transaction).toHaveBeenCalledOnce();
    expect(tx.character.update).toHaveBeenCalledOnce();
