...
describe('trade rules', () => {
  it('allows adjacent players on the same map', () => { expect(isTradeDistanceAllowed({ mapId: 'map-a', x: 10, y: 10 }, { mapId: 'map-a', x: 11, y: 11 })).toBe(true); });
  it('rejects players with a one-tile gap and players on another map', () => { expect(isTradeDistanceAllowed({ mapId: 'map-a', x: 10, y: 10 }, { mapId: 'map-a', x: 12, y: 10 })).toBe(false); expect(isTradeDistanceAllowed({ mapId: 'map-a', x: 10, y: 10 }, { mapId: 'map-b', x: 10, y: 10 })).toBe(false); });
