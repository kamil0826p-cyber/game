...
const activeGroupCombat = () => {
  ...
  const runtime = engine.createRequest(
    ...
    'map-a',
    team('first', [
      actor('first', { agility: 30, formationSlot: 0 }),
      actor('ally', { agility: 20, formationSlot: 1 }),
    ]),
    team('enemy-a', [
      actor('enemy-a', { agility: 15, formationSlot: 0 }),
      actor('enemy-b', { agility: 10, formationSlot: 1 }),
    ]),
    1_000,
    31_000,
  );
...
describe('CombatEngine', () => {
  ...
  it('attacks the explicitly selected legal front-row enemy', () => {
    const { engine, runtime } = activeGroupCombat();
    const snapshot = engine.act(
      runtime,
      'first',
      { action: 'BASIC_ATTACK', targetActorId: 'enemy-b' },
      2_000,
    );
  ...
  it('resolves AREA skills against every living enemy', () => {
    ...
    const snapshot = engine.act(
      runtime,
      'first',
      { action: 'SKILL', skillKey: 'mage-frost-nova' },
      2_000,
    );
    const enemies = snapshot.participants.filter((participant) =>
      ['enemy-a', 'enemy-b'].includes(participant.actorId),
    );
  ...
  it('finishes only after the complete enemy team is defeated', () => {
    ...
    const runtime = engine.createRequest(
      ...
      team('enemy-a', [
        actor('enemy-a', { hp: 1, maxHp: 1 }),
        actor('enemy-b', { hp: 1, maxHp: 1 }),
      ]),
      1_000,
      31_000,
    );
    ...
    expect(
      snapshot.participants
        .filter((participant) => participant.teamId === snapshot.teams?.[1].teamId)
        .every((participant) => participant.hp === 0),
    ).toBe(true);
  });
});
