...
const definition = { id: '22222222-2222-4222-8222-222222222222', key: 'rabbit-fur-for-mira', name: 'Ciepło dla Greenfields', description: 'Przynieś pięć futer.', minimumLevel: 1, steps: [{ id: 'fur', type: 'COLLECT_ITEM', itemKey: 'rabbit-fur', quantity: 5, consumeOnComplete: true }], rewards: { experience: 180, gold: 3, silver: 0 } };
...
function createService(furQuantity: number) {
  ...
  const transaction = {
    ...
    character: {
      ...
      update: vi.fn().mockResolvedValue({ id: characterQuest.characterId, level: 2, experience: 180, hp: 100, maxHp: 100, energy: 20, maxEnergy: 20, strength: 10, agility: 8, intelligence: 4, armor: 6, gold: 3, silver: 0, stateVersion: 2 }),
    },
...
describe('QuestService turn-in', () => {
  ...
  it('consumes items, closes the quest and grants rewards atomically', async () => {
    ...
    await expect(service.turnIn(player, definition.key, 'pl')).resolves.toMatchObject({ completed: true, state: 'REWARDED', reward: { experience: 180, gold: 3 } });
    expect(transaction.inventoryItem.delete).toHaveBeenCalled(); expect(transaction.characterCurrencyLedger.create).toHaveBeenCalledTimes(1); expect(player.gold).toBe(3);
  });
});
