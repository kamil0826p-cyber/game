...
describe('player trade socket schemas', () => {
  it('accepts a nearby-player trade request payload', () => {
  ...
  it('rejects gold, negative silver and duplicate item ids at the transport boundary', () => {
    expect(tradeOfferSchema.safeParse({ requestId, tradeId, gold: 1, silver: -1, items: [] }).success).toBe(false);
    const duplicate = tradeOfferSchema.safeParse({ requestId, tradeId, silver: 10, items: [{ itemId, quantity: 1 }, { itemId, quantity: 1 }] });
