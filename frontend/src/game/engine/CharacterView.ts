...
export class CharacterView {
  ...
  private frames: OutfitSheetFrames | undefined = undefined;
  private destroyed = false;
  private state: PublicPlayerState;
  private startX = 0; private startY = 0; private targetX = 0; private targetY = 0;
  private movementStartedAt = 0; private movementDuration = 1; private moving = false; private lastOutfitKey = '';
  ...
  constructor(state: PublicPlayerState, private readonly localPlayer: boolean, onInteract?: (player: PublicPlayerState, point: CharacterInteractionPoint) => void) {
    ...
    this.sprite.anchor.set(0.5, 1); this.sprite.scale.set(1.5); this.sprite.zIndex = 2; this.sprite.visible = false;
    this.nameText = new Text({ text: `${state.name}  Lv. ${state.level}`, style: { fontFamily: 'Inter, system-ui, sans-serif', fontSize: 12, fontWeight: '700', fill: localPlayer ? 0xfef3c7 : 0xf8fafc, stroke: { color: 0x05070d, width: 3 } } });
    ...
    this.nameplate.addChild(badge, this.nameText); this.nameplate.position.set(0, -72); this.nameplate.zIndex = 4;
    this.container.addChild(this.shadow, this.fallback, this.sprite, this.nameplate);
    const x = (state.x + 0.5) * WORLD_TILE_SIZE; const y = (state.y + 1) * WORLD_TILE_SIZE;
    this.startX = this.targetX = x; this.startY = this.targetY = y; this.container.position.set(x, y); this.loadOutfit(state.outfitKey);
  }
