...
export class CharacterView {
  ...
  private createFallback(state: PublicPlayerState): Graphics {
    ...
    return new Graphics().circle(0, -51, 10).fill({ color: 0xe8b98f }).roundRect(-12, -43, 24, 31, 7).fill({ color: primary }).rect(-11, -29, 22, 4).fill({ color: accent }).rect(-9, -14, 7, 14).fill({ color: 0x1f2937 }).rect(2, -14, 7, 14).fill({ color: 0x1f2937 });
  }
}
