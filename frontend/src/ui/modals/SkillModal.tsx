...
export function SkillModal({ onClose }: { onClose: () => void }): React.JSX.Element {
  return (
    <Modal title="Skill Tree" subtitle="Static progression concept" icon="✦" onClose={onClose} widthClass="max-w-3xl">
      <div className="relative h-[430px] overflow-hidden rounded-xl border border-white/10 bg-[radial-gradient(circle_at_center,rgba(124,58,237,0.12),rgba(2,6,23,0.75)_60%)]">
        ...
        {MOCK_SKILLS.map((skill) => (
          <button key={skill.id} type="button" className={`skill-node ${skill.rank > 0 ? 'skill-node-active' : ''}`} style={{ left: `${skill.x}%`, top: `${skill.y}%` }} title="Mock skill node">
            <span>✦</span><strong>{skill.label}</strong><small>Rank {skill.rank} / 5</small>
          </button>
        ))}
      </div>
      <p className="mock-banner mt-5">Skill points, prerequisites, effects, and server validation are future work.</p>
    </Modal>
  );
}
