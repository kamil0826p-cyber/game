...
export function ModalHost(): React.JSX.Element | null {
  ...
  useEffect(() => {
    const listener = (event: KeyboardEvent) => {
      if (
        ...
        state.activeModal === 'trade' ||
        state.activeModal === 'combat'
      )
        return;
  ...
  if (
    !state.self ||
    !state.activeModal ||
    ['trade', 'combat', 'npc-dialogue', 'merchant'].includes(state.activeModal)
  )
    return null;
  if (state.activeModal === 'character')
    return <CharacterModal character={state.self} onClose={close} />;
