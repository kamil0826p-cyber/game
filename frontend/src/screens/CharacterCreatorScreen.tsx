    ...
    <main className="auth-background h-dvh overflow-y-auto p-4 text-slate-100 sm:p-8">
      ...
      <form onSubmit={submit} className="mx-auto mt-6 grid max-w-6xl gap-6 lg:grid-cols-[1.05fr_0.95fr]">
        <Panel elevated className="p-5 sm:p-7">
              ...
              return (
                ...
                >
                  <OutfitPreview
                    ...
                    size="small"
                  />
                  <span className={`mt-2 font-display text-xl ${presentation.accent}`}>
                    {presentation.label}
                  </span>
          ...
          <div className="mt-6 rounded-xl border border-white/10 bg-slate-950/40 p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {startingStats.map((stat) => (
                <span key={stat} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-slate-300">
                  {stat}
                </span>
              ))}
            </div>
          ...
          {formError ? (
            <div role="alert" className="mt-4 rounded-lg border border-rose-500/40 bg-rose-950/50 p-3 text-sm text-rose-100">
              {formError}
            </div>
          ) : null}
        </Panel>
          ...
          <div className="relative">
            ...
            <div className="mt-5 flex items-center justify-between gap-3">
              ...
              <div className="text-center">
                ...
                <span className={`mt-3 inline-flex rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-wider ${preview.unlockLevel === 1 ? 'border-emerald-400/30 bg-emerald-500/10 text-emerald-200' : 'border-amber-400/30 bg-amber-500/10 text-amber-200'}`}>
                  {preview.unlockLevel === 1
                    ? t('character.unlocked')
                    : t('character.lockedAt', { level: preview.unlockLevel })}
                </span>
              </div>
              <button
                type="button"
