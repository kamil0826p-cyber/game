...
const GENDER_OPTIONS: ReadonlyArray<{
...
}> = [
  {
    value: 'MALE',
    label: 'Male',
    description: 'Stored on the character. Outfit artwork is selected by class and level.',
  },
  {
    value: 'FEMALE',
    label: 'Female',
    ...
    <main className="auth-background h-dvh overflow-y-auto p-4 text-slate-100 sm:p-8">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display mt-1 text-3xl text-amber-100">
            {t('character.createTitle')}
          </h1>
        </div>
      ...
      <form
        onSubmit={submit}
        className="mx-auto mt-6 grid max-w-6xl gap-6 lg:grid-cols-[1.05fr_0.95fr]"
      >
        <Panel elevated className="p-5 sm:p-7">
          ...
          {formError ? (
            <div
              role="alert"
              className="mt-4 rounded-lg border border-rose-500/40 bg-rose-950/50 p-3 text-sm text-rose-100"
            >
              {formError}
            </div>
          ) : null}
          ...
          <div className="relative">
            ...
            <div className="mt-5 flex items-center justify-between gap-3">
              ...
              <div className="text-center">
                ...
                <span className="mt-3 inline-flex rounded-full border border-emerald-400/30 bg-emerald-500/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-emerald-200">
            ...
            <p className="mt-5 rounded-lg border border-amber-400/20 bg-amber-500/5 p-3 text-xs leading-5 text-amber-100/80">
              Gender does not recolor outfits or change stats. The selected level 1 outfit is saved
              immediately.
            </p>
            <Button
