...
const genderLabel = (gender: CharacterRosterEntry['gender']): string =>
...
export function CharacterSelectScreen(): React.JSX.Element {
  ...
  const refresh = async () => {
    ...
    setSelectedId((current) =>
      roster.characters.some((entry) => entry.characterId === current)
        ? current
  ...
  const unlockedOutfits = useMemo(
    () =>
      selected
        ? OUTFIT_CATALOG[selected.characterClass].filter(
            (outfit) => outfit.unlockLevel <= selected.level,
          )
        : [],
    [selected],
  );
  ...
  const enterWorld = async () => {
    ...
    setError(null);
    try {
      if (state.self?.characterId !== selected.characterId) {
        await connection.selectCharacter(selected.characterId);
      }
  ...
  const changeOutfit = async (direction: number) => {
    ...
    const currentIndex = Math.max(
      0,
      unlockedOutfits.findIndex((outfit) => outfit.key === selected.outfitKey),
    );
    const next =
      unlockedOutfits[(currentIndex + direction + unlockedOutfits.length) % unlockedOutfits.length]!;
    setBusy(true);
    setError(null);
    try {
        ...
        <header className="flex items-start justify-between gap-4 border-b border-white/10 p-6 sm:p-8">
          <div>
            ...
            <h1 className="font-display mt-2 text-3xl text-amber-100">
              {t('character.selectTitle')}
            </h1>
            <p className="mt-2 text-sm text-slate-400">
              Choose one of your characters and prepare its outfit before entering.
            </p>
          </div>
        ...
        <section className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[0.72fr_1.28fr]">
          <aside>
              ...
              {characters.map((character) => (
                <button
                  ...
                  className={`flex items-center gap-3 rounded-xl border p-3 text-left transition ${
                    selected?.characterId === character.characterId
                      ? 'border-amber-400/60 bg-amber-500/10'
                      : 'border-white/10 bg-slate-950/35 hover:border-amber-400/30'
                  }`}
                >
                  <OutfitPreview
                  ...
                  />
                  <span>
                    <strong className="font-display block text-lg text-amber-100">
                      {character.name}
                    </strong>
                    <span className="text-xs text-slate-400">
                      Lv. {character.level} {t(classLabelKey[character.characterClass])} ·{' '}
            ...
            <Button
              ...
              type="button"
              disabled={characters.length >= maxCharacters || busy}
              onClick={() => setCreating(true)}
            >
              Create character
            </Button>
            {characters.length >= maxCharacters ? (
              <p className="mt-2 text-center text-xs text-slate-500">Character limit reached.</p>
            ) : null}
          </aside>
              ...
              <div>
                ...
                <div className="mt-4 flex items-center justify-between gap-3">
                  <button
                    ...
                    disabled={busy}
                    onClick={() => void changeOutfit(-1)}
                    aria-label="Previous outfit"
                  >
                    <OutfitArrow direction="left" />
                  </button>
                  <div className="text-center">
                    <p className="eyebrow">Outfit</p>
                    <h3 className="font-display text-lg text-amber-100">
                      {unlockedOutfits.find((outfit) => outfit.key === selected.outfitKey)?.label ??
                        selected.outfitKey}
                    </h3>
                    <p className="mt-1 text-xs text-slate-400">
                      {unlockedOutfits.length} of {OUTFIT_CATALOG[selected.characterClass].length}{' '}
                      unlocked
                    </p>
                  </div>
                  <button
              ...
              <div className="flex flex-col justify-center">
                ...
                <p
                  className={`mt-2 text-sm font-semibold uppercase tracking-[0.18em] ${
                    CLASS_PRESENTATION[selected.characterClass].accent
                  }`}
                >
                  {t('common.level')} {selected.level} {t(classLabelKey[selected.characterClass])}
                </p>
                <div className="mt-6 grid grid-cols-2 gap-3 text-sm">
                  <Info label="Gender" value={genderLabel(selected.gender)} />
                  <Info label={t('common.position')} value={`${selected.x}, ${selected.y}`} />
                  <Info label={t('common.outfit')} value={selected.outfitKey} />
                  <Info label="Experience" value={String(selected.experience)} />
                ...
                {error ? (
                  <p
                    role="alert"
                    className="mt-5 rounded-lg border border-rose-500/40 bg-rose-950/50 p-3 text-sm text-rose-100"
                  >
                    {error}
                  </p>
                ) : null}
                <Button
                  className="mt-8 justify-center py-3 text-base"
  ...
  return (
    <div>
      ...
      <div className="h-2 overflow-hidden rounded-full bg-slate-950">
        <div
          className={`h-full ${tone === 'health' ? 'bg-rose-500' : 'bg-sky-500'}`}
